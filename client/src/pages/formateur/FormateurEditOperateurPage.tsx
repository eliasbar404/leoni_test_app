...
const FormateurEditOperateurPage = () =>{
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      if (!id) return; // If there is no id in URL, do nothing

      try {
        const token = localStorage.getItem('token');
        const response = await axios.get(`http://localhost:3000/operateur/${id}`, {
        ...
        setProfileData({
          ...
          groupeId:response.data.user.groupeId
        });
  ...
  // };
  const handleProfileChange = (e: ChangeEvent<HTMLElement>) => {
    const target = e.target as HTMLInputElement | HTMLSelectElement;
    const { name, value } = target;
    setProfileData((prev) => 
      prev 
        ? { ...prev, [name]: name === 'groupeId' ? value : value } 
        : null
    );
  };
  ...
  const handleProfileSubmit = async (e: React.FormEvent) => {
    ...
    // Destructure profileData to check if any required fields are missing
    const { firstName, lastName, cin, matricule, phone, address } = profileData;
    ...
    try {
      ...
      formData.append('groupeId', profileData.groupeId || '');
      ...
      const response = await axios.put(`http://localhost:3000/operateur/${id}`, formData, {
        headers: {
      ...
      if (response.status === 200) {
        ...
        navigate('/formateur/dashboard/operateurs');
      }
    } catch (err) {
  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(
        `http://localhost:3000/operateur/${id}/password/update`,
        {
  ...
  <InputField
            ...
            onChange={handleProfileChange}
            placeholder="Entrez le Phone"
          />

          {/* Group */}
          <GroupSelect
                  name="groupeId"
                  value={profileData.groupeId??""}
                  onChange={handleProfileChange}
          />
        </div>
...
export default FormateurEditOperateurPage
