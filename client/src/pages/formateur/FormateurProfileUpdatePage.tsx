...
export default function FormateurProfileUpdatePagee() {
  ...
  const handleProfileSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.put('http://localhost:3000/formateur/profile/update', formData, {
        headers: {
      ...
      navigate('/formateur/dashboard/profile');
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    } catch (err) {
      Swal.fire({
        icon: "error",
  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(
        'http://localhost:3000/formateur/password/update',
        {
