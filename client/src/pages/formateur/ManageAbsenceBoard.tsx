...
function App() {
  ...
  const exportAttendance = () => {
    const data = filteredUsers.map(user => {
      ...
      return {
        ...
        Date: selectedDate,
        Status: attendanceRecord?.isPresent ? 'Présent' : 'Absent',
      };
    });
  ...
  // Modified calendar days calculation to handle Monday as first day
  ...
  const days = [
    ...Array(startingDayIndex).fill(null), // Add empty cells for proper alignment
    ...eachDayOfInterval({
          ...
          <div className="bg-white rounded-lg shadow">
            <div className="p-6">
              ...
              <div className="grid grid-cols-7 gap-px bg-gray-200">
                ...
                {days.map((day, index) => {
                  if (!day) {
                    return <div key={`empty-${index}`} className="bg-white p-3" />;
                  }
                      ...
                      {hasAttendance && isSameDay(day, new Date(selectedDate)) && (
                        <div className="mt-2">
                          <div className="text-xs text-gray-600 flex flex-col">
                            <span className='text-green-500 font-black'>{presentCount}/{totalCount} présent</span> 
                            <span className='text-red-500 font-black'>{totalCount - presentCount}/{totalCount} absent</span>
                          </div>
