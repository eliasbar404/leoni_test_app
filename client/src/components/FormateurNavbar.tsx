...
const FormateurNavbar = () => {
  const { profileData, loading, error } = useFormateurProfile();
  ...
  return (
    <nav className="fixed w-[100%] shadow-lg top-0 right-0 h-16 bg-white border-b border-gray-200 px-4 flex items-center justify-between z-40">
      <div className="flex items-center gap-4">
        <Link to='/formateur/dashboard/' className="text-2xl font-extrabold text-[#070a94]">LEONI</Link>
      </div>
...
const ProfileDropdown: React.FC<ProfileDropdownProps> = ({ onClose }) => {
  ...
  return (
    <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-50">
      <Link
        to="/formateur/dashboard/profile"
